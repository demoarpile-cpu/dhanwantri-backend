...
export const createClinicAdmin = async (clinicId: number, userData: any) => {
    ...
    const clinicIds: number[] = Array.isArray(userData.clinicIds) && userData.clinicIds.length > 0
        ? Array.from(new Set(userData.clinicIds.map((c: any) => Number(c)).filter(Boolean)))
        : [clinicId];
    ...
    for (const cid of clinicIds) {
        const exists = await prisma.clinicstaff.findFirst({
            where: { userId: user.id, clinicId: cid }
        });
        if (!exists) {
    ...
    await prisma.auditlog.create({
        data: {
            ...
            performedBy: 'SUPER_ADMIN',
            details: JSON.stringify({ userName: name, clinicIds, roles: finalRoles })
        }
    });
    ...
    return {
        ...
        email: user.email,
        role: primaryRole,
        roles: finalRoles.map((r: string) => r.toUpperCase()),
        clinics: clinicIds,
        status: user.status,
        joined: user.joined
    };
...
export const getAllStaff = async () => {
    ...
    staff.forEach((s: any) => {
        ...
        existing.clinics = Array.from(new Set([...(existing.clinics || []), s.clinicId]));
        existing.roles = Array.from(new Set([...(existing.roles || []), ...roles]));
        if (String(existing.role).toUpperCase() !== 'ADMIN' && String(s.role).toUpperCase() === 'ADMIN') {
            existing.role = 'ADMIN';
        }
    });
...
export const updateStaff = async (id: number, data: any) => {
    ...
    const { roles, role, clinicIds } = data;
    ...
    if (Array.isArray(clinicIds)) {
        const targetClinicIds = Array.from(new Set(clinicIds.map((c: any) => Number(c)).filter(Boolean)));
        const currentRecords = await prisma.clinicstaff.findMany({
            where: { userId: staff.userId }
        });
        const currentClinicIds = currentRecords.map(r => r.clinicId);

        const toAdd = targetClinicIds.filter(cid => !currentClinicIds.includes(cid));
        const toRemove = currentClinicIds.filter(cid => !targetClinicIds.includes(cid));

        if (toRemove.length > 0) {
            await prisma.clinicstaff.deleteMany({
                where: { userId: staff.userId, clinicId: { in: toRemove } }
            });
        }
        ...
        for (const cid of toAdd) {
            await prisma.clinicstaff.create({
                data: {
                    userId: staff.userId,
                    clinicId: cid,
                    role: (primaryRole || updatedStaff.role) as any,
                    roles: finalRoles ? JSON.stringify(finalRoles.map((r: string) => r.toUpperCase())) : updatedStaff.roles
                }
            });
        }
    }
    ...
    return {
        ...
        roles: returnRoles,
        clinics: Array.isArray(clinicIds) ? clinicIds.map((c: any) => Number(c)) : [updatedStaff.clinicId],
