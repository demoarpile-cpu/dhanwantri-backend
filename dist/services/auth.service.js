...
const buildAuthenticatedSession = async (user, ip, device, auditAction = '2FA Verification Success') => {
    ...
    if (isSuperAdmin) {
        ...
        if (roles.includes('ADMIN'))
            ...
            tokenRole = staffRecords[0].role; // Fallback to first role
    }
    else {
...
export const login = async (data, ip, device) => {
    ...
    if (isPatientRole) {
        ...
        return {
            success: true,
            otpRequired: true,
            ...(isOtpInlineDebug ? { devOtp: generatedOtp } : {}),
            user: {
                id: user.id,
                email: user.email,
                name: user.name,
...
export const selectClinic = async (userId, clinicId, role, ip, device) => {
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user)
        throw new AppError('User not found', 404);
